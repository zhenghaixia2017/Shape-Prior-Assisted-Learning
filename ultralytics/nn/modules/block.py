...
from .DeformableAttention import DeformableAttention2D
...
class DeformPSA(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, e: float = 0.5):
        ...
        self.attn = Attention(self.c, attn_ratio=0.5, num_heads=max(self.c // 64, 1))
        self.deformattn = DeformableAttention2D(self.c, downsample_factor = 2, dim_head = 64, heads = self.c//64, offset_kernel_size = 6)
        self.ffn = nn.Sequential(Conv(self.c, self.c * 2, 1), Conv(self.c * 2, self.c, 1, act=False))
        # self.ffn_a = nn.Sequential(Conv(self.c, self.c * 2, 1), Conv(self.c * 2, self.c, 1, act=False))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        a, b = self.cv1(x).split((self.c, self.c), dim=1)
        b = b + self.attn(b) + self.deformattn(a)
        b = b + self.ffn(b)
        return self.cv2(torch.cat((a, b), 1))
